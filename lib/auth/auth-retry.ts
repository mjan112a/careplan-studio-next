...
import { AUTH_RETRY } from './constants';

/**
 * Retry utility for authentication operations
 * Handles retrying operations with exponential backoff while respecting certain error types
 */
export const withRetry = async <T>(
  operation: () => Promise<T>,
  maxRetries = AUTH_RETRY.MAX_RETRIES,
  delay = AUTH_RETRY.BASE_DELAY
): Promise<T> => {
  let lastError: Error | undefined;
  
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      logger.debug('Attempting auth operation', { attempt, maxRetries });
      return await operation();
    } catch (error) {
      lastError = error as Error;
      logger.warn('Auth operation attempt failed', { 
        attempt, 
      ...
      if (attempt === maxRetries && error instanceof AuthError) {
        logger.error('Max retries reached with auth error', { 
          error: error.message,
          code: error.code
        });
        throw error;
      ...
      if (attempt < maxRetries) {
        ...
        logger.info('Retrying auth operation after delay', { attempt, backoffDelay });
  ...
  logger.error('All auth retry attempts failed', { 
    maxRetries,
    lastError: lastError instanceof Error ? lastError.message : String(lastError)
  });
  throw new AuthError(
    'Authentication operation failed after multiple retries',
